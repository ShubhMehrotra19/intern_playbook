      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <motion.div
            ...
            className="col-span-1 md:col-span-2 bg-linear-to-r from-indigo-900 to-purple-900 rounded-3xl p-8 relative overflow-hidden">
            <div className="relative z-10">
              <h1 className="text-3xl font-bold mb-2">Welcome Back! 🚀</h1>
              <p className="text-indigo-200 mb-6 max-w-lg">
                You have completed {user.completedTasks?.length || 0} out of{" "}
                {tasks.length} tasks. Keep pushing to climb the leaderboard!
              </p>
              ...
              <div className="mt-2 text-right text-sm font-medium text-green-300">
                {progress}% Complete
              </div>
            </div>
          ...
          <motion.div
            ...
            <p className="text-muted-foreground text-sm">
              Next level at {Math.ceil((user.xp + 1) / 500) * 500} XP
            </p>
          </motion.div>
        </div>
        ...
        <div>
          <h2 className="text-2xl font-bold mb-6 flex items-center">
            Your Mission
            <span className="bg-muted text-xs ml-3 px-2 py-1 rounded-full text-muted-foreground border border-border">
              {tasks.length} Tasks
            </span>
          </h2>
              ...
              {tasks.map((task, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`group relative bg-card rounded-2xl p-6 border transition-all hover:border-primary/50 shadow-sm ${completed
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                          <span className="text-xs bg-indigo-500/10 text-indigo-400 px-2 py-1 rounded font-medium">
                            +{task.xpReward} XP
                          </span>
                          {task.video && (
                            <span className="text-xs bg-red-500/10 text-red-400 px-2 py-1 rounded flex items-center">
                      ...
                      <button
                        onClick={() =>
                          !completed && handleComplete(task._id, task.xpReward)
                        }
                        disabled={completed}
                        className={`p-3 rounded-xl transition-all ${completed
